...
class Certifications(BaseModel):
    certifications: List[Certification] = Field(
        description="job relevant certifications that you have earned, including the name, issuing organization, and a link to verify the certification."
    )


class Education(BaseModel):
    degree: str = Field(
        description="The degree or qualification obtained and The major or field of study. e.g., Bachelor of Science in Computer Science."
    )
    university: str = Field(
        description="The name of the institution where the degree was obtained with location. e.g. Arizona State University, Tempe, USA"
    )
    ...
    courses: List[str] = Field(
        description="Relevant courses or subjects studied during the education period. e.g. [Data Structures, Algorithms, Machine Learning]"
    )
...
class Project(BaseModel):
    ...
    type: str | None = (
        Field(
            description="The type or category of the project, such as hackathon, publication, professional, and academic."
        ),
    )
    link: str = Field(description="A link to the project repository or demo.")
    resources: Optional[List[Link]] = Field(
        description="Additional resources related to the project, such as documentation, slides, or videos."
    )
...
class SkillSections(BaseModel):
    skill_section: List[SkillSection] = Field(
...
class Experience(BaseModel):
    ...
    description: List[str] = Field(
        description="A list of 3 bullet points describing the work experience, tailored to match job requirements. Each bullet point should follow the 'Did X by doing Y, achieved Z' format, quantify impact, implicitly use STAR methodology, use strong action verbs, and be highly relevant to the specific job. Ensure clarity, active voice, and impeccable grammar."
    )
...
class ResumeSchema(BaseModel):
    ...
    media: Media = Field(
        description="Links to professional social media profiles, such as LinkedIn, GitHub, or personal website."
    )
    work_experience: List[Experience] = Field(
        description="Work experiences, including job title, company, location, dates, and description."
    )
