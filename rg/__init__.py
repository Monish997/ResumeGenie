...
from rg.prompts.resume_prompt import (
    CV_GENERATOR,
    JOB_DETAILS_EXTRACTOR,
    RESUME_DETAILS_EXTRACTOR,
    RESUME_WRITER_PERSONA,
)
from rg.schemas.job_details_schema import JobDetails
from rg.schemas.sections_schemas import ResumeSchema
from rg.utils import utils
from rg.utils.data_extraction import extract_text, read_data_from_url
from rg.utils.latex_ops import latex_to_pdf
from rg.utils.llm_models import ChatGPT, Gemini, OllamaModel
from rg.utils.metrics import (
    cosine_similarity,
    jaccard_similarity,
    overlap_coefficient,
    vector_embedding_similarity,
)
from rg.variables import DEFAULT_LLM_MODEL, DEFAULT_LLM_PROVIDER, LLM_MAPPING, section_mapping
...
class AutoApplyModel:
    ...
    def __init__(
        ...
        provider: str = None,
        model: str = None,
        downloads_dir: str = utils.get_default_download_folder(),
        system_prompt: str = RESUME_WRITER_PERSONA,
    ):
        self.system_prompt = system_prompt
        self.provider = (
            DEFAULT_LLM_PROVIDER if provider is None or provider.strip() == "" else provider
        )
        self.model = DEFAULT_LLM_MODEL if model is None or model.strip() == "" else model
        self.downloads_dir = (
            utils.get_default_download_folder()
    ...
    def cover_letter_generator(
        self, job_details: dict, user_data: dict, need_pdf: bool = True, is_st=False
    ):
        """
        ...
        try:
            ...
            for section in [
                ...
                prompt = PromptTemplate(
                    ...
                    section_data=json.dumps(user_data[section]),
            ...
            for metric in ["jaccard_similarity", "overlap_coefficient", "cosine_similarity"]:
                ...
                if metric == "vector_embedding_similarity":
                    ...
                    job_alignment = globals()[metric](
                        llm, json.dumps(resume_details), json.dumps(job_details)
                    )
